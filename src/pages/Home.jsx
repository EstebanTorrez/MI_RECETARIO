...
useEffect(() => {
  const fetchData = async () => {
    setLoading(true);
    try {
      const recipeData = await getRecipes(page);
      setRecipes((prevRecipes) => [...prevRecipes, ...recipeData.results]);
      setHasMore(recipeData.results.length > 0); // Si hay resultados, hay más recetas
      setLoading(false);
    } catch (error) {
      setError(error.message);
      setLoading(false);
    ...
    <Container>
      ...
      <main>
        {loading ? (
          ...
          {hasMore && (
            <div className="text-center">
              <Button onClick={() => setPage((prevPage) => prevPage + 1)} variant="primary">
                Load More
              </Button>
            </div>
          )}
